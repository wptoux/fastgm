cdef sm4_memcpy(dst, dst_offset, src, src_offset, len):
    cdef uint _
    for _ in range(len):
...
cdef SM4_GETU32(data, offset):
    return ((data[offset] << 24)
            | (data[offset + 1] << 16)
            | (data[offset + 2] << 8)
            | data[offset + 3])
    
cdef SM4_PUTU32(data, offset, value):
...
cdef int SM4_KEY_LENGTH = 16
cdef int SM4_BLOCK_SIZE = 16
cdef int SM4_IV_LENGTH = SM4_BLOCK_SIZE
cdef int SM4_NUM_ROUNDS = 32
